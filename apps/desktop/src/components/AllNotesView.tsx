...
import { StickyNote, Search, X, Folder, FileText, Trash2, ChevronDown, Filter } from 'lucide-react';
...
export function AllNotesView() {
  const { allNotes, noteGroups, projects, loadAllNotes, selectNote, deleteNote } = useStore();
  ...
  const isFiltered =
    !!search || sourceFilter !== 'all' || selectedGroupIds !== null || selectedProjectIds !== null;
        ...
        {filteredNotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-surface-500">
            ...
            <p className="text-sm font-medium mb-1">
              {allNotes.length === 0 ? 'No notes yet' : 'No notes match your search'}
            </p>
...
function AllNotesCard({ note, group, project, searchQuery, onClick, onDelete }: AllNotesCardProps) {
  ...
  const renderHighlighted = (text: string) => {
    ...
    while (cursor < text.length) {
      ...
      parts.push(
        <mark key={idx} className="bg-accent-500/30 text-accent-200 rounded px-0.5">
    ...
    <label className="flex items-center gap-2 px-3 py-1.5 text-xs text-surface-300 hover:bg-surface-800/60 transition-all cursor-pointer">
      ...
      <span
        className={clsx(
          ...
          checked ? 'bg-accent-600 border-accent-600' : 'border-surface-600',
        )}
      >
        {checked && (
