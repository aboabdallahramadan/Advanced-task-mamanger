...
function createFocusWidget() {
  ...
  focusWidget = new electron.BrowserWindow({
    width: 420,
    height: 52,
...
if (!gotTheLock) {
  ...
  electron.app.whenReady().then(async () => {
    const dbPath2 = path.join(electron.app.getPath("userData"), "tmap.db");
...
⏱ ${data.elapsed}`);
  ...
  electron.ipcMain.on("focus:showWidget", () => {
    createFocusWidget();
  });
  electron.ipcMain.on("focus:hideWidget", () => {
    if (focusWidget && !focusWidget.isDestroyed()) {
      focusWidget.close();
      focusWidget = null;
    }
  });
  electron.ipcMain.on("focus:widgetState", (_e, data) => {
    if (focusWidget && !focusWidget.isDestroyed()) {
      focusWidget.webContents.send("focus:state", data);
    }
  });
  electron.ipcMain.on("focus:widgetAction", (_e, action) => {
