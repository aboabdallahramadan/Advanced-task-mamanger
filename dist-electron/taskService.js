...
const recurrenceUtils_1 = require("./recurrenceUtils");
...
function queryTasks(db, sql, params = []) {
    ...
    const tasks = result[0].values.map((row) => rowToTask(columns, row));
    // Batch-load all subtasks for these tasks
    if (tasks.length > 0) {
        const taskIds = tasks.map((t) => t.id);
        const placeholders = taskIds.map(() => '?').join(',');
        const subResult = db.exec(`SELECT * FROM subtasks WHERE task_id IN (${placeholders}) ORDER BY sort_order ASC, created_at ASC`, taskIds);
        if (subResult.length > 0) {
            const subColumns = subResult[0].columns;
            const subtasks = subResult[0].values.map((row) => rowToSubtask(subColumns, row));
            const subtaskMap = new Map();
            for (const st of subtasks) {
                if (!subtaskMap.has(st.taskId))
...
class TaskService {
    ...
    getAll() {
        return queryTasks(this.db, 'SELECT * FROM tasks WHERE status != ? AND is_recurrence_template = 0 ORDER BY sort_order ASC, created_at DESC', ['archived']);
    }
    getByDate(date) {
    ...
    update(id, updates) {
        ...
        if (updates.recurrenceDetached !== undefined) {
            sets.push('recurrence_detached = ?');
            values.push(updates.recurrenceDetached ? 1 : 0);
        }
        if (sets.length === 0)
            return this.getById(id);
    ...
    getUpcomingWithReminders() {
        return queryTasks(this.db, `SELECT * FROM tasks
    ...
    search(query) {
        return queryTasks(this.db, `SELECT * FROM tasks WHERE (title LIKE ? OR notes LIKE ?) AND status != ? AND is_recurrence_template = 0 ORDER BY sort_order ASC LIMIT 50`, [`%${query}%`, `%${query}%`, 'archived']);
    }
    // ─── Subtask Operations ─────────────────────────────────
    ...
    createRecurringTask(taskInput, ruleInput) {
        const now = new Date().toISOString();
        const ruleId = (0, uuid_1.v4)();
        // Create recurrence rule
        this.db.run(`INSERT INTO recurrence_rules (id, frequency, interval_value, days_of_week, end_type, end_count, end_date, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ruleId,
            ruleInput.frequency,
    ...
    generateInstancesForRange(ruleId, startDate, endDate) {
        const rule = this.getRecurrenceRule(ruleId);
        if (!rule)
            return [];
        // Find the template task
        const templateResult = queryTasks(this.db, 'SELECT * FROM tasks WHERE recurrence_rule_id = ? AND is_recurrence_template = 1', [ruleId]);
        if (templateResult.length === 0)
            return [];
        const template = templateResult[0];
        // Get existing instance dates
        const existingResult = this.db.exec('SELECT recurrence_original_date FROM tasks WHERE recurrence_rule_id = ? AND is_recurrence_template = 0 AND recurrence_original_date IS NOT NULL', [ruleId]);
        ...
        // Get exception dates
        const excResult = this.db.exec('SELECT exception_date FROM recurrence_exceptions WHERE recurrence_rule_id = ?', [ruleId]);
        const exceptionDates = new Set();
        if (excResult.length > 0) {
            for (const row of excResult[0].values) {
                if (row[0])
                    exceptionDates.add(row[0]);
            }
        }
        const ruleData = {
    ...
    ensureInstancesForDateRange(startDate, endDate) {
        // Find all rules that need generation extended
        const result = this.db.exec(`SELECT id FROM recurrence_rules WHERE generated_until IS NULL OR generated_until < ?`, [endDate]);
        if (result.length === 0)
            return [];
        const allNew = [];
    ...
    updateRecurrenceRule(ruleId, ruleUpdates) {
        ...
        const sets = [];
        const values = [];
        if (ruleUpdates.frequency !== undefined) {
            sets.push('frequency = ?');
            values.push(ruleUpdates.frequency);
        }
        if (ruleUpdates.interval !== undefined) {
            sets.push('interval_value = ?');
            values.push(ruleUpdates.interval);
        }
        if (ruleUpdates.daysOfWeek !== undefined) {
            sets.push('days_of_week = ?');
            values.push(JSON.stringify(ruleUpdates.daysOfWeek));
        }
        if (ruleUpdates.endType !== undefined) {
            sets.push('end_type = ?');
            values.push(ruleUpdates.endType);
        }
        if (ruleUpdates.endCount !== undefined) {
            sets.push('end_count = ?');
            values.push(ruleUpdates.endCount);
        }
        if (ruleUpdates.endDate !== undefined) {
            sets.push('end_date = ?');
            values.push(ruleUpdates.endDate);
        }
        if (sets.length === 0)
            return;
        ...
        this.db.run(`DELETE FROM tasks WHERE recurrence_rule_id = ? AND is_recurrence_template = 0 AND recurrence_detached = 0 AND planned_date >= ? AND status != 'done'`, [ruleId, today]);
        // Re-generate
        const twoWeeks = new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0];
        this.generateInstancesForRange(ruleId, today, twoWeeks);
        (0, database_1.saveDatabase)();
