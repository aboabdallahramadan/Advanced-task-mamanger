...
import { ListChecks, Search, ChevronDown, X, ArrowUpDown, Filter, Layers } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-surface-950">
      ...
      <div className="px-6 py-3 border-b border-surface-800/40 flex items-center gap-2 flex-wrap">
        ...
        <FilterDropdown label="Priority" count={selectedPriorities.size} total={5}>
                ...
                {groupBy !== 'none' && (
                  <div className="sticky top-0 z-10 px-6 py-2 bg-surface-950/95 backdrop-blur-sm border-b border-surface-800/30">
                    <div className="flex items-center gap-2">
                      {groupBy === 'status' && (
                        <span
                          className={clsx(
                            ...
                            STATUS_COLORS[group.key] || 'bg-surface-600',
                          )}
                        />
                      )}
                      ...
                      <span className="text-xs font-semibold text-surface-300">{group.label}</span>
