    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {(() => {
            const dayTasks = tasks.filter(
              (t) => t.plannedDate === selectedDate && t.status !== 'archived',
            );
          ...
          <span
            className={clsx(
              ...
              freeMinutes > 120
                ...
                : freeMinutes > 0
                  ? 'chip-warning'
                  : 'bg-danger-900/40 text-danger-300 border-danger-700/30',
            )}
          >
            {freeMinutes > 0
          ...
          {conflicts.size > 0 && (
            <div className="flex items-center gap-1.5 text-xs text-warning-400">
              ...
              <span>
                {Math.floor(conflicts.size / 2)} conflict{conflicts.size > 2 ? 's' : ''}
              </span>
            </div>
          )}
        </div>
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto relative custom-scrollbar">
        ...
        >
          {/* Hour lines */}
          {hours.map((hour) => (
            <HourLine key={hour} hour={hour} top={(hour - workStartHour) * HOUR_HEIGHT} />
          ))}
          ...
          {/* Current time indicator */}
          {currentTimeTop > 0 &&
            currentTimeTop < (workEndHour - workStartHour + 1) * HOUR_HEIGHT && (
              <div
...
function TimeBlock({
  ...
  const startTime = task.scheduledStart ? format(parseISO(task.scheduledStart), 'h:mm a') : '';
  const endTime = task.scheduledEnd ? format(parseISO(task.scheduledEnd), 'h:mm a') : '';
      ...
      <div className="flex items-start justify-between h-full overflow-hidden">
        <div className="flex-1 min-w-0">
          <p
            ...
            style={{ ...getDirectionStyle(task.title), color: projectColor }}
          >
            {task.title}
          </p>
          {currentHeight > 40 && (
        ...
        {hasConflict && <AlertTriangle className="w-3 h-3 text-warning-400 flex-shrink-0 mt-0.5" />}
      </div>
