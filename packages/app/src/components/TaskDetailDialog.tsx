...
export function TaskDetailDialog() {
  ...
  const [recurrenceActionDialog, setRecurrenceActionDialog] = useState<{
    mode: 'edit' | 'delete';
    pendingData?: Partial<Task>;
  } | null>(null);
  ...
  const descriptionEditor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'focus:outline-none min-h-[60px] max-h-[150px] overflow-y-auto px-4 py-2.5 text-sm text-surface-100',
      },
    },
  });
  ...
  useEffect(() => {
    ...
    if (taskDialog.mode === 'edit' && taskDialog.taskId) {
      ...
      if (task) {
        ...
        setScheduledStart(
          task.scheduledStart ? task.scheduledStart.split('T')[1]?.substring(0, 5) || '' : '',
        );
  ...
  const currentTask =
    isEdit && taskDialog.taskId ? tasks.find((t) => t.id === taskDialog.taskId) : null;
  ...
  const handleRecurrenceAction = async (scope: 'this' | 'future' | 'all') => {
    ...
    if (recurrenceActionDialog.mode === 'edit' && recurrenceActionDialog.pendingData) {
      if (scope === 'this') {
        ...
        await updateRecurrenceSeries(
          currentTask.recurrenceRuleId,
          recurrenceActionDialog.pendingData,
        );
      }
      ...
      if (scope === 'this') {
        ...
        await deleteRecurrenceSeriesFuture(
          currentTask.recurrenceRuleId,
          currentTask.plannedDate || format(new Date(), 'yyyy-MM-dd'),
        );
      } else if (scope === 'all') {
          ...
          {isEdit && currentTask?.recurrenceRuleId && (
            <div className="flex items-center gap-2 px-3 py-2 bg-accent-600/10 border border-accent-500/20 rounded-xl">
              ...
              <span className="text-xs text-accent-300">
                {currentTask.recurrenceDetached
                  ? 'Detached from recurring series'
                ...
                {subtasks.map((st) => (
                  <div
                    ...
                    className={clsx(
                      ...
                      st.completed ? 'bg-transparent' : 'hover:bg-surface-800/40',
                    )}
                  >
                    <button
                    ...
                    {editingSubtaskId === st.id ? (
                      ...
                      <span
                        ...
                        className={clsx(
                          ...
                          st.completed ? 'line-through text-surface-600' : 'text-surface-200',
                        )}
                      >
                        {st.title}
                      </span>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-surface-800 bg-surface-900">
          <div className="flex items-center gap-2">
            <span className="text-xs text-surface-500">
              <kbd className="text-surface-400 bg-surface-800 px-1.5 py-0.5 rounded text-2xs">
                Ctrl+Enter
              </kbd>{' '}
              to save
            </span>
