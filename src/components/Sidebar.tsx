  ...
  const getCounts = (id: ViewMode) => {
    switch (id) {
      ...
      case 'backlog':
        return backlogCount;
      case 'all':
        return deduplicateRecurring(tasks.filter((t) => t.status !== 'archived')).length;
      default:
        return 0;
    }
  };
    ...
    <div className="w-56 bg-surface-950 border-r border-surface-800/60 flex flex-col animate-slide-in">
      ...
      <div className="px-4 pt-12 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm text-surface-200 tracking-tight">
            Daily Planner
          </span>
        </div>
        <button
      ...
      <nav
        className="px-2 py-0 flex flex-col gap-0.5"
        role="navigation"
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setCurrentView(item.id)}
              className={clsx('sidebar-item', currentView === item.id && 'sidebar-item-active')}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="px-4 py-2">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              <ChevronDown
                className={clsx('w-3 h-3 transition-transform', projectsCollapsed && '-rotate-90')}
              />
              Projects
            </button>
            <button
        ...
        <div className="px-4 py-2">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              <ChevronDown
                className={clsx('w-3 h-3 transition-transform', notesCollapsed && '-rotate-90')}
              />
              Notes
            </button>
            <button
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  {noteGroups.map((group) => {
                    const isActive =
                      currentView === 'noteGroup' && selectedNoteGroupId === group.id;
